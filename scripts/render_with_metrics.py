...
def post_render_qc(project_id,job_id,obj):
 ...
 with tempfile.TemporaryDirectory() as d:
  raw=Path(d)/'raw.mp4';master=Path(d)/'master.mp4';download_output(obj,raw);longform=int(p.get('target_duration_seconds') or 0)>120 or str(p.get('format') or '').lower() in ('long','longform','full','youtube','youtube video','full video','long form','long-form');add_sound_design(raw,master,longform);upload(master,'video-outputs',obj,'video/mp4');step(p,'sound_design','passed','Added a subtle locally generated ambience bed, narration-safe compression and final loudness mastering.')
  thumb_obj=None
  if longform:
   thumb=Path(d)/'thumbnail.jpg';make_thumbnail(master,p.get('title'),thumb);thumb_obj=f"{p['user_id']}/{p['id']}/{job_id}.jpg";upload(thumb,'video-thumbnails',thumb_obj,'image/jpeg');step(p,'thumbnail','passed','Generated a custom 16:9 thumbnail from the finished video with concise title treatment.')
  result=final_video_qc(master,p,assets)
 ...
 patch('video_projects',p['id'],payload);step(p,'final_video_qc','passed' if result['passed'] else 'failed',f"Finished-video score {result['score']}/100. "+('; '.join(result['reasons']) if result['reasons'] else 'Actual MP4 passed motion, freeze, black-frame, silence, duration, aspect-ratio, sound-master and integrity checks.'))
