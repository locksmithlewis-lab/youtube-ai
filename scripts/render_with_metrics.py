...
def report(project, job, stage, result):
    return req('POST', '/rest/v1/video_quality_reports', {
        ...
        'render_job_id': job.get('id') if job else None,
        'stage': stage,
...
def prepare_longform(project):
    ...
    try:
        ...
        patch('video_projects', project['id'], {
            'script': made['script'], 'hook': made['hook'], 'updated_at': 'now()',
        })
...
def preflight_queue():
    ...
    for job in jobs:
        ...
        try:
            ...
            reason = 'Long-form writer failed: ' + str(exc)
        ...
        detail = f"Creative score {result['score']}/100. " + (
            '; '.join(result['reasons']) if result['reasons'] else
            'Hook, spoken script, title, rhythm, originality and instruction-leak checks passed.'
        )
...
def add_sound_design(src, dst, longform=False):
    dur = max(1.0, media_duration(src))
    bed = .55 if longform else .75
    filt = (
        f"[1:a]lowpass=f=420,highpass=f=45,volume=0.07,afade=t=in:st=0:d={bed},"
        f"afade=t=out:st={max(0, dur - bed):.3f}:d={bed}[bed];"
        "[0:a][bed]amix=inputs=2:duration=first:weights='1 0.34',"
        "acompressor=threshold=-16dB:ratio=1.6:attack=8:release=180,"
        "loudnorm=I=-14:TP=-1:LRA=8[mix]"
    )
    subprocess.run([
        'ffmpeg', '-y', '-i', str(src), '-f', 'lavfi', '-i',
        f'anoisesrc=color=pink:amplitude=0.025:sample_rate=48000:d={dur:.3f}',
...
def make_thumbnail(video, title, path):
    ...
    timecode = max(1, min(dur * .28, dur - 1))
    text = path.with_suffix('.txt')
    text.write_text(' '.join(str(title or 'Video').split()[:10]), encoding='utf-8')
    font = '/usr/share/fonts/truetype/dejavu/DejaVuSans-Bold.ttf'
    vf = (
        'scale=1280:720:force_original_aspect_ratio=increase,crop=1280:720,'
        'eq=contrast=1.08:saturation=1.12,drawbox=x=0:y=430:w=1280:h=290:color=black@0.48:t=fill,'
        f'drawtext=fontfile={font}:textfile={text}:fontcolor=white:fontsize=58:line_spacing=10:x=70:y=470:box=0'
    )
    subprocess.run([
        'ffmpeg', '-y', '-ss', f'{timecode:.2f}', '-i', str(video), '-frames:v', '1',
        '-vf', vf, '-q:v', '2', str(path),
    ], check=True, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
...
def fail_finished_qc(project, job, reason, result=None):
    ...
    patch('video_projects', project['id'], {
        'status': 'failed', 'output_url': None, 'quality_score': (result or {}).get('score', 0),
        'failure_reason': reason[:1000], 'updated_at': 'now()',
    })
    patch('render_jobs', job['id'], {
        'status': 'failed', 'error': reason[:1000], 'updated_at': 'now()',
    })
    step(project, 'final_video_qc', 'failed', reason[:1000])


def post_render_qc(project_id, job_id, obj):
    project = (req('GET', f'/rest/v1/video_projects?id=eq.{project_id}&select=*') or [None])[0]
    job = (req('GET', f'/rest/v1/render_jobs?id=eq.{job_id}&select=*') or [None])[0]
    ...
    try:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            master = Path(temp_dir) / 'master.mp4'
            download_output(obj, raw)
            longform = (
                int(project.get('target_duration_seconds') or 0) > 120
                or str(project.get('format') or '').lower() in ('long', 'longform', 'full', 'youtube', 'youtube video', 'full video', 'long form', 'long-form')
            )
            add_sound_design(raw, master, longform)
            step(project, 'sound_design', 'passed', 'Added a subtle locally generated ambience bed, narration-safe compression and final loudness mastering.')

            result = final_video_qc(master, project, assets)
            report(project, job, 'final_video_qc', result)
            if not result['passed']:
                reason = 'Final video QC failed: ' + '; '.join(result['reasons'])
                patch('video_projects', project['id'], {
                    'quality_score': result['score'], 'status': 'failed', 'output_url': None,
                    'failure_reason': reason, 'updated_at': 'now()',
                })
                patch('render_jobs', job['id'], {'status': 'failed', 'error': reason, 'updated_at': 'now()'})
                step(project, 'final_video_qc', 'failed', f"Finished-video score {result['score']}/100. " + '; '.join(result['reasons']))
                raise RuntimeError(reason)

            upload(master, 'video-outputs', obj, 'video/mp4')
            thumb_obj = None
            if longform:
                thumb = Path(temp_dir) / 'thumbnail.jpg'
                make_thumbnail(master, project.get('title'), thumb)
                thumb_obj = f"{project['user_id']}/{project['id']}/{job_id}.jpg"
                upload(thumb, 'video-thumbnails', thumb_obj, 'image/jpeg')
                step(project, 'thumbnail', 'passed', 'Generated a custom 16:9 thumbnail from the finished video with concise title treatment.')

            creative = float(project.get('creative_score') or 0)
            priority = publication_priority(project, creative, result['score'])
            payload = {
                'quality_score': result['score'], 'publication_priority': priority,
                'status': 'quality_check', 'failure_reason': None, 'updated_at': 'now()',
            }
            if thumb_obj:
                payload['thumbnail_url'] = thumb_obj
            patch('video_projects', project['id'], payload)
            step(project, 'final_video_qc', 'passed', (
                f"Finished-video score {result['score']}/100. Actual MP4 passed semantic relevance, motion, "
                'freeze, black-frame, silence, duration, aspect-ratio and sound-master checks.'
            ))
        ...
        if current.get('status') != 'failed':
            fail_finished_qc(project, job, 'Final video QC execution failed: ' + str(exc))
        raise


preflight_queue()
render = subprocess.run(['python', 'scripts/render_video.py'], capture_output=True, text=True)
text = (render.stdout or '') + (render.stderr or '')
print(text, end='')
