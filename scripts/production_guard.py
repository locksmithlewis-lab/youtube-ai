...
def creative_preflight(project):
 ...
 score=max(0,round(score,1));return {'passed':score>=78 and not any('leaked' in r for r in reasons),'score':score,'reasons':reasons,'metrics':{'words':len(ws),'sentences':len(ss),'title_words':len(words(title)),'duplicate_sentence_ratio':round(dup,3),'target_word_range':[low,high]}}
def _ffprobe(path):
...
def final_video_qc(path,project,assets):
 ...
 score=max(0,round(score,1));metrics={'duration_seconds':round(dur,2),'size_bytes':size,'width':w,'height':h,'motion_ratio':round(motion_ratio,3),'image_ratio':round(image_ratio,3),'provider_count':providers,**detect};return {'passed':score>=82 and not reasons,'score':score,'reasons':reasons,'metrics':metrics}
