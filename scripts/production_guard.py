...
def sentences(text):return [re.sub(r'\s+',' ',s).strip() for s in re.split(r'(?<=[.!?])\s+|\n+',str(text or '')) if len(s.strip().split())>2]
def words(text):return re.findall(r"[A-Za-z0-9']+",str(text or '').lower())
def _target_words(project):
 target=max(1,int(project.get('target_duration_seconds') or 60));fmt=str(project.get('format') or '').lower()
...
def instruction_leaks(script):
 ...
 for pat in INSTRUCTION_PATTERNS:
  m=re.search(pat,script or '')
  if m:found.append(m.group(0))
 return found
def creative_preflight(project):
 script=str(project.get('script') or '').strip();hook=str(project.get('hook') or '').strip();title=str(project.get('title') or '').strip();ss=sentences(script);ws=words(script);reasons=[];score=100.0;low,high=_target_words(project)
 ...
 if len(words(title))<3:reasons.append('title is too vague');score-=10
 if len(title)>100:reasons.append('title is too long for YouTube packaging');score-=7
 if re.search(r'(?i)^(?:showcase|signature)\s*\d+',title):reasons.append('generic batch title detected');score-=22
 if re.search(r'(?i)\b(?:viral|must watch|you won.t believe|crazy)\b',title):score-=4
 leaked=False
 for pat in OUTLINE_MARKERS:
  if re.search(pat,script):leaked=True;break
 leaks=instruction_leaks(script)
 if leaked or leaks:
  sample=', '.join(leaks[:3]) if leaks else 'outline/production marker'
  reasons.append(f'production directions leaked into spoken script ({sample})');score-=60
 ...
 normalized=[re.sub(r'\W+',' ',s.lower()).strip() for s in ss];dup=1-(len(set(normalized))/max(1,len(normalized)))
 ...
 score=max(0,round(score,1));return {'passed':score>=78 and not leaked and not leaks,'score':score,'reasons':reasons,'metrics':{'words':len(ws),'sentences':len(ss),'title_words':len(words(title)),'duplicate_sentence_ratio':round(dup,3),'target_word_range':[low,high],'instruction_leak_count':len(leaks)}}
def _ffprobe(path):
...
def _detect(path):
 p=subprocess.run(['ffmpeg','-hide_banner','-nostats','-i',str(path),'-vf','blackdetect=d=0.35:pix_th=0.10,freezedetect=n=-45dB:d=1.0','-af','silencedetect=n=-48dB:d=2.5','-f','null','-'],stdout=subprocess.DEVNULL,stderr=subprocess.PIPE,text=True);text=p.stderr or '';black=[float(x) for x in re.findall(r'black_duration:([0-9.]+)',text)];freeze=[float(x) for x in re.findall(r'freeze_duration: ([0-9.]+)',text)];silence=[];starts=[float(x) for x in re.findall(r'silence_start: ([0-9.]+)',text)];ends=[float(x) for x in re.findall(r'silence_end: ([0-9.]+)',text)]
...
def final_video_qc(path,project,assets):
 path=Path(path);reasons=[];score=100.0;meta=_ffprobe(path);fmt=meta.get('format') or {};stream=(meta.get('streams') or [{}])[0];dur=float(fmt.get('duration') or 0);size=int(fmt.get('size') or 0);w=int(stream.get('width') or 0);h=int(stream.get('height') or 0);target=float(project.get('target_duration_seconds') or dur or 1);kind=str(project.get('format') or '').lower();longform=target>120 or kind in ('long','longform','full','youtube','youtube video','full video','long form','long-form');motion=sum(1 for a in assets if a.get('media_type') in ('video','graphic'));images=sum(1 for a in assets if a.get('media_type')=='image');motion_ratio=motion/max(1,len(assets));image_ratio=images/max(1,len(assets));providers=len({a.get('provider') for a in assets if a.get('provider')});detect=_detect(path)
 rel=[float(a.get('relevance_score') or 0) for a in assets];avg_rel=sum(rel)/max(1,len(rel));low_rel=sum(1 for x in rel if x<.46);low_rel_ratio=low_rel/max(1,len(rel))
 ...
 if avg_rel<.50:reasons.append(f'average semantic visual relevance only {avg_rel:.2f}; narration and visuals do not align closely enough');score-=30
 if len(assets)>=8 and low_rel_ratio>.25:reasons.append(f'{low_rel_ratio:.0%} of scenes are weak semantic matches');score-=25
 ...
 score=max(0,round(score,1));metrics={'duration_seconds':round(dur,2),'size_bytes':size,'width':w,'height':h,'motion_ratio':round(motion_ratio,3),'image_ratio':round(image_ratio,3),'provider_count':providers,'average_semantic_relevance':round(avg_rel,3),'weak_semantic_scene_ratio':round(low_rel_ratio,3),**detect};return {'passed':score>=82 and not reasons,'score':score,'reasons':reasons,'metrics':metrics}
