...
ENGINE='motion-first-renderer-v10-retention'; VOICE_MODEL=os.environ.get('PIPER_VOICE','en_US-lessac-medium'); VOICE_DIR=Path(os.environ.get('PIPER_VOICE_DIR','.piper-voices'))
...
def video_filter(i=0):return f"scale={W}:{H}:force_original_aspect_ratio=increase,crop={W}:{H},eq=contrast=1.035:saturation=1.05,setsar=1"
def image_motion_filter(i,frames):
 zoom="min(zoom+0.00075,1.11)" if i%2==0 else "if(lte(zoom,1.0),1.10,max(1.0,zoom-0.00065))";x="iw/2-(iw/zoom/2)+18*sin(on/20)";y="ih/2-(ih/zoom/2)+12*cos(on/24)";return f"scale={int(W*1.16)}:{int(H*1.16)}:force_original_aspect_ratio=increase,crop={int(W*1.16)}:{int(H*1.16)},zoompan=z='{zoom}':x='{x}':y='{y}':d={frames}:s={W}x{H}:fps=30,eq=contrast=1.04:saturation=1.06,setsar=1"
def make_graphic(path,seconds,index,plan):
 keys=(plan.get('keywords') or [])[:3];c=plan.get('character') or {};label=(f"{c.get('name','')}  {c.get('accessory','')}" if c else '  •  '.join(keys))[:80] or ' ';txt=path.with_suffix('.txt');txt.write_text(label,encoding='utf-8');font='/usr/share/fonts/truetype/dejavu/DejaVuSans-Bold.ttf';fs=44 if H>W else 38;vf=(f"drawgrid=width=120:height=120:thickness=2:color=white@0.055,"f"drawbox=x='-260+mod(t*170+{index*90},{W+420})':y='{int(H*.22)}':w=260:h=18:color=white@0.22:t=fill,"f"drawbox=x='{int(W*.08)}+40*sin(t*1.7)':y='{int(H*.38)}+70*cos(t*1.15)':w='{int(W*.28)}':h='{int(H*.17)}':color=white@0.055:t=fill,"f"drawbox=x='{int(W*.62)}+55*cos(t*1.3)':y='{int(H*.58)}+90*sin(t*.9)':w='{int(W*.22)}':h='{int(H*.12)}':color=white@0.045:t=fill,"f"drawtext=fontfile={font}:textfile={txt}:fontcolor=white@0.90:fontsize={fs}:x=(w-text_w)/2:y=h*0.72:box=1:boxcolor=black@0.18:boxborderw=18,vignette=PI/5");run(['ffmpeg','-y','-f','lavfi','-i',f'color=c=0x0b1320:s={W}x{H}:r=30:d={seconds:.3f}','-vf',vf,'-c:v','libx264','-preset','veryfast','-crf','19','-pix_fmt','yuv420p',str(path)],stdout=subprocess.DEVNULL,stderr=subprocess.DEVNULL)
...
def render_asset(asset,plan,i,seg,frames,work):
 ...
 if asset['media_type']=='graphic':make_graphic(src,max(seg+.3,3.0),i,plan);run(['ffmpeg','-y','-i',str(src),'-t',f'{seg:.3f}','-an','-c:v','libx264','-preset','veryfast','-crf','19','-pix_fmt','yuv420p',str(clip)],stdout=subprocess.DEVNULL,stderr=subprocess.DEVNULL);return clip,asset
