...
def motion_filter(motion,frames):
 ...
 dx="sin(on/22)*9" if motion=='drift-left' else ("-sin(on/25)*9" if motion=='drift-right' else "sin(on/29)*4")
...
def visual_qc(real,total,unique_ids,fallbacks):
 ratio=real/max(1,total); unique_ratio=len(unique_ids)/max(1,real)
 reasons=[]
