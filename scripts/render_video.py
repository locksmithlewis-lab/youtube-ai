...
ENGINE='motion-first-renderer-v11-continuous-motion'; VOICE_MODEL=os.environ.get('PIPER_VOICE','en_US-lessac-medium'); VOICE_DIR=Path(os.environ.get('PIPER_VOICE_DIR','.piper-voices'))
...
def video_filter(i=0):return f"scale={W}:{H}:force_original_aspect_ratio=increase,crop={W}:{H},fps=30,setpts=N/(30*TB),eq=contrast=1.035:saturation=1.05,setsar=1"
...
def final_motion_filter():
 sw=int(W*1.018)//2*2;sh=int(H*1.018)//2*2
 return f"scale={sw}:{sh},crop={W}:{H}:x='(in_w-out_w)/2+((in_w-out_w)/2)*sin(t*0.55)':y='(in_h-out_h)/2+((in_h-out_h)/2)*cos(t*0.43)',fps=30,noise=alls=1.2:allf=t,setsar=1"
...
def render_asset(asset,plan,i,seg,frames,work):
 ...
 try:run(['ffmpeg','-y','-stream_loop','-1','-ss','0.8','-i',str(src),'-t',f'{seg:.3f}','-an','-vf',video_filter(i),'-c:v','libx264','-preset','veryfast','-crf','19','-pix_fmt','yuv420p',str(clip)],stdout=subprocess.DEVNULL,stderr=subprocess.DEVNULL);return clip,asset
 except Exception:return render_asset(local_graphic_asset(plan,i),plan,i,seg,frames,work)
