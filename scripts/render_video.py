...
ENGINE='rolixa-multisource-renderer-v8-expressive'; VOICE_MODEL=os.environ.get('PIPER_VOICE','en_US-lessac-medium'); VOICE_DIR=Path(os.environ.get('PIPER_VOICE_DIR','.piper-voices'))
...
def download(url,path):
 for attempt in range(3):
  try:
   req=urllib.request.Request(url,headers={'User-Agent':'RolixaVisualRouter/2.2','Accept':'*/*'})
  ...
  except Exception:time.sleep(.75*(attempt+1))
 return False
...
def qc(assets,total):
 scores=[float(a.get('relevance_score') or 0) for a in assets];avg=sum(scores)/max(1,len(scores));ids={a.get('id') for a in assets};providers={a.get('provider') for a in assets};graphics=sum(a.get('media_type')=='graphic' for a in assets);reasons=[];warnings=[]
 ...
 if total>=8 and len(providers)<2:warnings.append('single visual provider used; accepted because relevance passed')
 if graphics>math.ceil(total*.55):reasons.append('generated graphics dominate the edit')
 return reasons,warnings,avg,providers
...
def expressive_voice(script,work,model):
 ss=sentences(script);parts=[];scales=[.94,1.04,.98,1.08,.92,1.00];pauses=[.08,.14,.06,.18,.10,.12]
 for i,s in enumerate(ss):
  text=work/f'voice-{i:02}.txt';wav=work/f'voice-{i:02}.wav';text.write_text(s,encoding='utf-8')
  ...
  parts.append(wav)
  if i<len(ss)-1:
   gap=work/f'pause-{i:02}.wav';run(['ffmpeg','-y','-f','lavfi','-i','anullsrc=r=22050:cl=mono','-t',str(pauses[i%len(pauses)]),'-c:a','pcm_s16le',str(gap)],stdout=subprocess.DEVNULL,stderr=subprocess.DEVNULL);parts.append(gap)
...
try:
 ...
 for s,d in [('voice',f'Generating expressive narration with {VOICE_MODEL} and varied pacing.'),('visuals','Matching narration beats across multiple providers when useful.'),('edit','Building safe captions and cinematic pacing.')]:set_step(project,s,'running',d)
 ...
 audio=expressive_voice(script,work,model);dur=duration(audio);ss=sentences(script);scene_count=max(8,min(22,math.ceil(dur/3.1)));seg=dur/scene_count;copies=[ss[min(len(ss)-1,math.floor(i*len(ss)/scene_count))] for i in range(scene_count)]
 ...
 for i,line in enumerate(copies):
  ...
  if not asset:asset=choose(plan,used,.42,used_providers)
 ...
 reasons,warnings,avg,providers=qc(assets,scene_count)
 ...
 elapsed=max(.1,time.monotonic()-started);patch('render_jobs',job['id'],{'status':'completed','engine':ENGINE,'output_url':obj,'error':None,'completed_at':'now()','actual_render_seconds':elapsed,'media_duration_seconds':dur,'updated_at':'now()'});patch('video_projects',project['id'],{'output_url':obj,'voice':VOICE_MODEL,'status':'quality_check','failure_reason':None,'updated_at':'now()'})
 warn=(' '+ '; '.join(warnings)) if warnings else '';set_step(project,'voice','passed',f'Expressive narration generated with {VOICE_MODEL}; sentence pacing varies naturally.');set_step(project,'visuals','passed',f"Visual relevance QC passed: {scene_count} scenes, avg relevance {avg:.2f}, providers {', '.join(sorted(providers))}.{warn}");set_step(project,'edit','passed','Safe-area captions, varied narration pacing and decode validation passed.');print(f'Rendered {obj}: voice={VOICE_MODEL}, relevance={avg:.2f}, providers={providers}, engine={ENGINE}')
