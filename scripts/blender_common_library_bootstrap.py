...
def apply(_scene):
    ...
    # Shared Poly Haven HDRI lighting for identical reflections/exposure across workers.
    ...
    # Shared physical texture influences the industrial steel material in every segment.
    ...
    # All workers choose from the exact same four locked plates; no per-worker regeneration.
    ...
    scene["blackstar_common_library_version"]=1
    print(f"BLACKSTAR common library applied to segment {idx}: {plate if plate else 'no plate'}")
