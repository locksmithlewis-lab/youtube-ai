...
def score_clip(path,length):
    ...
    energy=round((audio_presence*.55+dynamic*.45)*35,2);activity=round(active_ratio*30,2);visual=round(motion*25,2);pacing=round(length_fit*10,2)
    score=round(min(100,energy+activity+visual+pacing),1)
    return score,{'audio_energy':energy,'active_audio':activity,'visual_change':visual,'length_fit':pacing,'mean_db':mean_db,'max_db':max_db,'scene_changes':changes,'active_ratio':round(active_ratio,3)}
...
try:
    ...
    pipe(project['id'],'visuals','running','Capturing the authorized source section.');pipe(project['id'],'edit','running','Cutting, formatting, and scoring the highlight.')
    ...
    req('POST','/rest/v1/render_jobs',{'user_id':job['user_id'],'project_id':project['id'],'engine':'rolixa-stream-clipper-v3-score','status':'completed','output_url':obj if accepted else None,'created_at':job['created_at'],'started_at':started.isoformat(),'completed_at':done.isoformat(),'media_duration_seconds':length,'actual_render_seconds':actual,'updated_at':done.isoformat()},{'Prefer':'return=minimal'})
    pipe(project['id'],'visuals','passed' if accepted else 'failed','Authorized clip formatted successfully.' if accepted else f'Clip discarded: highlight score {score}/{threshold}.');pipe(project['id'],'edit','passed' if accepted else 'failed',f'Highlight score {score}/100. '+('Kept.' if accepted else 'Rejected and not stored.'));pipe(project['id'],'quality_check','passed' if accepted else 'failed',f'Entertainment clip score {score}/100; threshold {threshold}.');pipe(project['id'],'ready','passed' if accepted else 'blocked','Ready for posting.' if accepted else 'Weak clip automatically discarded.')
    if accepted:
    ...
    print(f'Clip {decision}: score={score}, render={actual:.1f}s')
