...
try:
    ...
    pipe(project['id'],'visuals','running','Downloading only the authorized source section needed for this clip.');pipe(project['id'],'edit','running','Cutting and formatting the selected highlight.')
    ...
    req('POST','/rest/v1/render_jobs',{'user_id':job['user_id'],'project_id':project['id'],'engine':'rolixa-stream-clipper-v1','status':'completed','output_url':obj,'created_at':job['created_at'],'started_at':started.isoformat(),'completed_at':done.isoformat(),'media_duration_seconds':length,'actual_render_seconds':actual,'updated_at':done.isoformat()},{'Prefer':'return=minimal'})
