...
btn?.addEventListener('click', async () => {
  btn.disabled=true; message.textContent='Finding the strongest live trend…';
  try{
    ...
    const trend=(trendBody.trends||[])[0]; if(!trend) throw new Error('No live trend was available.');
    message.textContent='Writing the story and queuing the visual edit…';
    ...
    const {error:renderError}=await supabase.from('render_jobs').insert({user_id:user.id,project_id:project.id,status:'queued'}); if(renderError) throw renderError;
    message.textContent=`Queued: “${generated.title}”. Story-first narration and animated visual scenes will be rendered.`;
