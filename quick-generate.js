...
function buildShort(trend){
  const topic=cleanTitle(trend.topic),views=Number(trend.evidence?.views||0),channel=String(trend.evidence?.channel||'the channel').trim();
  const hook=`Okay, ${topic} just hit the feed like a jump scare.`;
  const proof=views?`By the time Rolixa caught it, ${channel} was already sitting around ${compactNumber(views)} views.`:`It had already broken into YouTube's most-popular feed.`;
  const script=[hook,proof,`Now forget the spreadsheet for a second. This is the moment where a normal upload turns into something people keep sending to each other because nobody wants to be the last person who has not seen it.`,`You click once, then suddenly the comments are flying, the counter keeps moving, and the whole thing feels bigger every time you refresh.`,`Maybe it keeps climbing. Maybe the internet finds a new obsession by dinner. That is the fun part: right now nobody knows.`,`So if ${topic} is about to become the thing everybody is talking about, you are catching the wave while it is still forming.`,`Would you watch it now, or wait to see if the hype survives?`].join(' ');
  return {hook,script,title:`${topic}: catch the wave before it moves on`.slice(0,140),format:'Short',style:'Hype',duration:45};
}
...
async function generateTrendVideo(kind){
  ...
  const factual=kind==='documentary';
  await supabase.from('project_pipeline_steps').insert([
    {user_id:user.id,project_id:project.id,step:'research',status:factual?'running':'passed',detail:factual?'Factual claims require verified sources before release.':'Entertainment video: verified research is not required unless factual claims are added.'},
    {user_id:user.id,project_id:project.id,step:'script',status:'passed',detail:factual?'Evidence-first documentary narration created.':'Entertainment-first narration created for pace, curiosity, and payoff.'},
...
function goStudio(target){closeChooser();document.querySelector('[data-view="studio"]')?.click();setTimeout(()=>document.getElementById(target)?.scrollIntoView({behavior:'smooth',block:'start'}),180);}
function openChooser(){
  ...
  body.querySelector('[data-qg="series"]').onclick=()=>goStudio('seriesForm');
  body.querySelector('[data-qg="clip"]').onclick=()=>goStudio('clipStudioPanel');
  body.querySelector('[data-qg="watch"]').onclick=()=>goStudio('watchForm');
  body.querySelector('[data-qg="manual"]').onclick=()=>goStudio('projectForm');
}
