...
const STOP=new Set(['this','that','with','from','have','will','your','they','them','then','into','about','while','where','when','what','video','chapter','episode','story','original','rolixa','the','and','for','are','but','not','you','was','were','has','had','its','our','their','generated','artificial','intelligence']);
function titleCase(s){return String(s||'').split(/\s+/).filter(Boolean).map(x=>x[0]?.toUpperCase()+x.slice(1).toLowerCase()).join('');}
function add(arr,value){const t=cleanToken(value);if(t.length>=3&&!arr.some(x=>x.toLowerCase()===t.toLowerCase())&&!['rolixa','rolixaoriginal','aigenerated','generatedbyai'].includes(t.toLowerCase()))arr.push(t);}
...
function appendHashtags(description,project,max=55){const tags=generateHashtags(project,max);const base=String(description||project.topic||'Original short video').replace(/\bRolixa\b/gi,'').replace(/\s{2,}/g,' ').trim();const room=Math.max(0,5000-base.length-2);let line='';for(const t of tags){const next=(line?line+' ':'')+t;if(next.length>room)break;line=next;}return {description:(base+(line?'\n\n'+line:'')).slice(0,5000),hashtags:tags};}
